...
int events_pull_messages()
{
    ...
    if (timeout_sec > MAX_PULLMESSAGES_TIMEOUT_SEC || limit > MAX_EVENTS) {
        ...
        log_error("PullMessages request exceeds device limits (timeout %d > %d sec, limit %d > %d)",
                  timeout_sec, MAX_PULLMESSAGES_TIMEOUT_SEC, limit, MAX_EVENTS);
